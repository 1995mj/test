/* global and local scope 전역과 지역 스코프
   전역은 코드의 가장 바깥 영역을 말하며 전역은 전역 스코프를 만든다.
   전역에 변수를 선언하면 전역 스코프를 갖는 전역변수가 되며 전역변수는 어디서든지 참조할 수 있다.
...
// console.log(z);  // z if not defined

/* 스코프 체인
   
   전역 스코프(x, y, outer) <- outer 지역 스코프 (z, inner) <- inner 지역 스코프(x)

   모든 스코프는 하나의 계층적 구조로 연결되며, 모든 지역 스코프 최상위 스코프는 전역 스코프이다.
